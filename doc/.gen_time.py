...
elapsed = np.array([4.39, 8.70, 20.19])
active = np.array([0.39, 8.70, 10.19, ])
wait = elapsed - active
...
for i, (a, w, total) in enumerate(zip(active, wait, elapsed)):
    ...
    if w > 0.05:
        ...
        ax.text(i, y_pos_wait, f"{w:.2f}s wait",
...
plt.margins(y=0.1)
plt.tight_layout()
plt.savefig("images/test1_execution_time.png", dpi=200)
plt.show()